...
### Requirement: FastAPI service persists board state to PostgreSQL
The system SHALL read and write board state from a Neon PostgreSQL database via `DATABASE_URL`. The JSON file store is removed. All route handlers SHALL be fully async.

#### Scenario: Board persists across service restarts
- **WHEN** a work item is created and the API process is restarted
- **THEN** the board including the new item is returned on the next `GET /kanban`, loaded from PostgreSQL

#### Scenario: Database session is injected per request
- **WHEN** any route handler processes a request
- **THEN** it receives a fresh `AsyncSession` via FastAPI dependency injection and commits or rolls back before the response is sent
